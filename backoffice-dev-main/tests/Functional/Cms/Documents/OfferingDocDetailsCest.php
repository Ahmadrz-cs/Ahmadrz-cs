...
class OfferingDocDetailsCest
{
    ...
    {
        $I->amOnPage('/admin/offeringdocument/1/edit');

        $description = bin2hex(random_bytes(8)) . 'description';
        $tag = bin2hex(random_bytes(8)) . 'tag';

        //check fields can be edited
        $I->fillField('input#offering_document_document_description', $description);
        $I->fillField('input#offering_document_document_tag', $tag);
        $I->click('button#offering_document_submit');

        $I->amOnPage('/admin/offeringdocument/1/edit');
        $I->seeInField('input#offering_document_document_description', $description);
        $I->seeInField('input#offering_document_document_tag', $tag);
