...
final class InvestmentDocumentRepositoryTest extends FixtureTestCase
{
    private InvestmentDocumentRepository $service;

    protected function setUp(): void
    {
        parent::setUp();
        $this->service = static::getContainer()->get(InvestmentDocumentRepository::class);
    }
        ...
        foreach ($results as $object) {
            foreach ($filters as $key => $expected) {
                ...
                if (in_array($key, ['investmentId'])) {
                    $key = lcfirst(substr($key, strlen('investment')));
    ...
    {
        yield 'Investment relation' => [['investmentId' => '4']];
        yield 'Investment relation multi' => [['investmentId' => [1, 4]]];
        yield 'Document relation' => [['documentTag' => 'share_certificate']];
