<?php

namespace App\Form;

use App\Entity\AssetStatusLog;
use App\Entity\Enum\AssetStatus;
use Symfony\Component\Form\AbstractType;
use Symfony\Component\Form\Extension\Core\Type\DateTimeType;
use Symfony\Component\Form\Extension\Core\Type\EnumType;
use Symfony\Component\Form\Extension\Core\Type\TextType;
use Symfony\Component\Form\FormBuilderInterface;
use Symfony\Component\OptionsResolver\OptionsResolver;

class StatusLogType extends AbstractType
{
    public function buildForm(FormBuilderInterface $builder, array $options): void
    {
        $builder->add('status', EnumType::class, [
            'class' => $options['status_class'],
            'disabled' => $builder->getData()->getId() ? true : false,
            // 'expanded' => true,
            'help' => 'What status the ' . $options['parent_entity_name'] . ' is in',
            'label' => 'Status',
            'placeholder' => 'Choose a status',
        ])->add('occuredAt', DateTimeType::class, [
            'help' => 'The date and time the status change was applied. Defaults to now, but can be changed for retrospective actions.',
            'widget' => 'single_text',
            'with_seconds' => true,
        ])->add('notes', TextType::class, [
            'attr' => [
                'placeholder' => 'State a reason for the state change',
            ],
            'help' => 'Optional but for manual state changes, it is recommended to provide a reason.',
            'required' => false,
        ]);
    }

    public function configureOptions(OptionsResolver $resolver): void
    {
        $resolver->setDefaults([
            'data_class' => AssetStatusLog::class,
            'status_class' => AssetStatus::class,
            'parent_entity_name' => 'asset',
        ]);
    }
}
